...
function NextHalfHour() { 
  ...
  if (nextHalfHourTime.isSame(agora, 'day')) { 
    text = `Daqui a pouco`; 
  } else {
    text = `Amanhã`;
  }
...
export default function Calendar(props) {
  const nextHalfHour = NextHalfHour(); 
    ...
    <>
      {picker !== 'calendar' ? (
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              onClick={() => {
                ...
                props.onSelectDateTime({ datetime: nextHalfHour.datetime });
              }}
              ...
              <CoffeeIcon className="text-teal-600 h-14 w-14 group-active:text-white" strokeWidth={1} /> 
              <div className="flex flex-col text-center">
                <p className="text-sm font-bold group-active:text-white">{nextHalfHour.text}</p>
            ...
            <button
              onClick={() => {
                ...
                props.onSelectDateTime({ datetime: nextDay.datetime });
              }}
              className="w-full gap-2 flex-col transition-all duration-100 rounded-xl hover:border-white/20 hover:bg-[#27272A]/20 flex items-center justify-center py-2 hover:shadow-lg border-transparent border active:bg-teal-400 group active:scale-95">
            ...
            <button
              onClick={() => {
                ...
                props.onSelectDateTime({ datetime: nextSaturday.datetime });
              }}
              className="w-full gap-2 flex-col transition-all duration-100 rounded-xl hover:border-white/20 hover:bg-[#27272A]/20 flex items-center justify-center py-2 hover:shadow-lg border-transparent border active:bg-teal-400 group active:scale-95">
            ...
            <button
              onClick={() => {
                ...
                props.onSelectDateTime({ datetime: nextMonday.datetime });
              }}
              className="w-full gap-2 flex-col transition-all duration-100 rounded-xl hover:border-white/20 hover:bg-[#27272A]/20 flex items-center justify-center py-2 hover:shadow-lg border-transparent border active:bg-teal-400 group active:scale-95">
            ...
            {window && window.lastSelection && window.lastSelection.datetime && (
              <button
                onClick={() => {
                  props.onSelectDateTime({ datetime: window.lastSelection.datetime });
                }}
                className="w-full gap-2 flex-col transition-all duration-100 rounded-xl hover:border-white/20 hover:bg-[#27272A]/20 flex items-center justify-center py-2 hover:shadow-lg border-transparent border active:bg-teal-400 group active:scale-95">
        ...
        <motion.div
          ...
          initial={{ opacity: 0, scale: 0.5, transform: 'translateX(100px)' }} 
            ...
            {days.map((day, dayIdx) => (
              <div key={day.toString()} className={clsx(dayIdx === 0 && colStartClasses[getDay(day)], 'py-1.5')}>
                <button
                  ...
                  className={clsx(
                    isEqual(day, selectedDateTime.date) && 'text-teal-400', 
                    !isEqual(day, selectedDateTime.date) && isToday(day) && 'text-teal-600', 
                    !isEqual(day, selectedDateTime.date) && 
                      !isToday(day) && 
                    ...
                    !isEqual(day, selectedDateTime.date) && 
                      ...
                      !isSameMonth(day, firstDayCurrentMonth) && 
                      'text-gray-600',
                    isEqual(day, selectedDateTime.date) && isToday(day) && 'bg-teal-600/50', 
                    isEqual(day, selectedDateTime.date) && !isToday(day) && 'bg-teal-950', 
                    !isEqual(day, selectedDateTime.date) && 'hover:bg-teal-950 hover:text-white', 
                    (isEqual(day, selectedDateTime.date) || isToday(day)) && 'font-semibold', 
          ...
          <div className="flex w-full gap-4 mt-2">
            <Button
              isDisabled={!selectedDateTime.date || !selectedDateTime.time} 
              size="md"
              className="w-full text-base bg-teal-500"
              radius="md"
              onClick={() => {
                ...
                props.onSelectDateTime({ datetime: `${date} ${time}` });
              }}>
              Confirmar
              <strong>
